    ...
    T: std::fmt::Display + Copy + Ord
{
    ...
    fn solve(mut self) -> String {
        ...
        self.dfs(&mut output, &mut compress_arr, &mut Vec::with_capacity(self.m), 0, 0);
    ...
    fn dfs(
        ...
        if self.m == depth {
            ...
            for j in i..compress_arr.len() {
                if compress_arr[j].1 != 0 {
                    compress_arr[j].1 -= 1;
                    temp_arr.push(compress_arr[j].0);
                    self.dfs(output, compress_arr, temp_arr, j, depth + 1);
                    temp_arr.pop();
                    compress_arr[j].1 += 1;
                }
            }
