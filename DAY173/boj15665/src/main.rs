    ...
    T: std::fmt::Display + Copy + Ord
{
    ...
    fn solve(mut self) -> String {
        ...
        self.dfs(&mut output, &unique_arr, &mut Vec::with_capacity(self.m), 0);
    ...
    fn dfs(
        ...
        if self.m == depth {
            ...
            for i in 0..unique_arr.len() {
                temp_arr.push(unique_arr[i]);
                self.dfs(output, unique_arr, temp_arr, depth + 1);
                temp_arr.pop();
            }
        }
    }
